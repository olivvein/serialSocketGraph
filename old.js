...
function serialData(data, port) {
	dataS=mapChiffre(data,minIn,maxIn,minOut,maxOut);
	...
	if (data.indexOf('ok') == 0) {
		...
		emitToPortSockets(port, 'serialRead', dataS);
		...
		// error is red
		emitToPortSockets(port, 'serialRead', dataS);		
		//emitToPortSockets(port, 'serialRead', {'line':'<span style="color: red;">RESP: '+dataS+'</span>'});
		...
		// other is grey
		//emitToPortSockets(port, 'serialRead', {'line':'<span style="color: #888;">RESP: '+dataS+'</span>'});
		emitToPortSockets(port, 'serialRead', dataS);
		//sendToApi(dataS);
	}
